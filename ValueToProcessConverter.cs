...
namespace MikuMikuVLC
{
    class ValueToProcessConverter : IValueConverter
    {
        ...
        {
            NormalBrush = new SolidColorBrush(Color.FromRgb(242, 68, 68));
            WarnBrush = new SolidColorBrush(Color.FromRgb(242, 68, 68));
        ...
        private void DrawingGeometry(DrawingContext drawingContext, double value, double maxValue, double radiusX, double radiusY, double thickness, double padding)
        {
            if (value != maxValue)
            {
                ...
                //.FromRgb(0xdd, 0xdf, 0xe1)
                drawingContext.DrawEllipse(null, new Pen(new SolidColorBrush(Colors.Transparent), thickness), centerPoint, radiusX, radiusY);
